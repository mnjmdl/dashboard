...
export default function EditAssetForm({ isOpen, onClose, onAssetUpdated, asset }: EditAssetFormProps) {
  ...
  useEffect(() => {
    if (asset) {
      setFormData({
        ...
        status: asset.status,
        assignedToId: asset.assignedToId ? asset.assignedToId.toString() : ''
      });
    }
  }, [asset]);
